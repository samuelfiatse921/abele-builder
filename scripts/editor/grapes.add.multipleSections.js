/**
 * Add Multiple Section Functionality for GrapesJS with Click and Drag-and-Drop Support
 * Adds a section with columns via click or drag-and-drop of #addMultipleSectionTool
...
(function () {
  ...
  if (window.addSectionFunctionalityLoaded) {
    console.log(
      "Add Section functionality already loaded, skipping initialization."
    );
    return;
  }
  window.addSectionFunctionalityLoaded = true;
  ...
  const componentConfigs = {
    section: {
      type: "default",
      tagName: "section",
      attributes: { class: "multiple-sections" },
      style: {
        display: "flex",
        "flex-direction": "row",
        gap: "20px",
  ...
  const addMultipleSectionTool = document.getElementById(
    "addMultipleSectionTool"
  );
  ...
  if (addMultipleSectionTool) {
    // Click event
    addMultipleSectionTool.addEventListener("click", () => {
      console.log("Click event triggered on #addMultipleSectionTool");
      addComponentToEditor("section");
    });

    // Drag-and-drop events
    addMultipleSectionTool.setAttribute("draggable", true);
    addMultipleSectionTool.addEventListener("dragstart", (e) => {
      e.dataTransfer.setData("text/plain", "addMultipleSectionTool");
      console.log("Dragging section tool with ID: addMultipleSectionTool");
    });

    addMultipleSectionTool.addEventListener("dragend", () => {
      console.log("Drag ended for section tool, over canvas:", isOverCanvas);
      isOverCanvas = false;
    });
  } else {
    console.error(
      "Add multiple section tool element not found. Please check the selector '#addMultipleSectionTool'."
    );
  }
  ...
  grapeEditor.on("component:add", (component) => {
    ...
    if (content === "addMultipleSectionTool") {
      console.log(
        "Replacing dropped text 'addMultipleSectionTool' with section component"
      );
  ...
  console.log("Multiple Section components loaded successfully!");
})();
